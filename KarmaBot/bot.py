...
def setSessionCode(code, update):
    ...
    db.collection(u'Session').document(str(update["message"]["from"]["id"])).set(data)

def getSessionCode(update):
    doc_ref = db.collection(u'Session').document(str(update["message"]["from"]["id"]))
    try:
        doc = doc_ref.get()
...
def process_update(update):
    ...
    if "/start" == update["message"]["text"]:
        ...
        if (getIntakeCode(update) != ''):
            process_html_message(update, prettyCSV.getTimetable(getIntakeCode(update)))
        else:
            process_html_message(update, "Please set your intake code with /setintake first.")
        setSessionCode('tt', update)

    elif regex_tt.match(update["message"]["text"]) != None:
        if (getIntakeCode(update) != ''):
            process_html_message(update, prettyCSV.getDayTimetable(getIntakeCode(update), regex_tt.match(update["message"]["text"]).group().replace('/tt', '')))
        else:
            process_html_message(update, "Please set your intake code with /setintake first.")
        setSessionCode('tt', update)
