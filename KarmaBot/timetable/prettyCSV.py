...
def getTimetable(intakeCode):
	...
	with open("timetable/2018-10-08.csv", newline='') as csv_file:
	    ...
	    if intakeCode == "UC2F1805CS-NWS":
	    	for row in csv_reader:
	    		if (row[1].upper() == "UC2F1805CS"):
	        		if datetime.datetime.today().strftime('%d-%m-%Y') in row[2]:
	        			dateToPrint = row[2]
	        			if row[6] not in ignoredModule:
	        				message += ('<strong>Module Code:</strong> <em>' + row[6] + '\n' + '</em><strong>Time:</strong> <em>' + row[3] + '\n' + '</em><strong>Location:</strong> <em>' + row [5] + '\n' + '</em><strong>Lecturer:</strong> <em>' + row [7] + '</em>\n\n')
	    else:
	    	for row in csv_reader:
	...
	if message != '':
		print('Message sent!')
	return '<strong>Date:</strong> <em>' + dateToPrint + '</em>\n\n' + message

def getDayTimetable(intakeCode, dayOfWeek):
	message = ''
	if dayOfWeek == '1':
		dayOfWeek = 'MON'
	elif dayOfWeek == '2':
